...
@dataclass(kw_only=True)
class ImprovedDistractorInfo(DistractorInfo):
    improved_distractor: str
    improved_reason: str


def improve_distractor(
        handler: AiHandler,
        context: str,
        distractor: DistractorInfo,
) -> ImprovedDistractorInfo | None:
    prompt = get_settings()["distractor_remake_sentence_with_reason_prompt"]
    response = asyncio.run(
        run_prompt(
            handler,
            prompt,
            user_vars={
                "correct_sentence": context,
                "wrong_sentence": distractor.distractor,
                "wrong_reason": distractor.reason,
            },
            system_vars=dict(),
        )
    )
    ...
    try:
        ...
        reason = data.get("explanation", "").strip()

        if sentence and reason:
            return ImprovedDistractorInfo(
                distractor=distractor.distractor,
                reason=distractor.reason,
                improved_distractor=sentence,
...
if __name__ == "__main__":
    ...
    for context in [
        ...
        for distractor in data.distractors:
            improved = improve_distractor(handler, context, distractor)
            if improved:
                pp.pprint(context)
                pp.pprint(improved)
